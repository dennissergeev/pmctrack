...
use types, only: wp
use constants, only: pi, ra, rkilo, fillval, ifillval, &
                   & nmax, pmax, mx, my, deg2rad
...
do
! TODO: make it a separate subroutine (the code repeats below)
  p = 0
...
do i_max = 1, n_max
  if (remove_flag(i_max)) then
    ...
    do j = 0, ny
      do i = 0, nx
        if(vor_part(i, j) == i_max) vor_part(i, j) = ifillval
      enddo
    enddo
  else
...
do i_max = 1, n_max_out
  ...
  do
      ...
      do i = 0, nx
        if (vor_part_tmp(i, j) == i_max) then
          do i_max2 = 1, n_max_out
            if (      buf_mij_out(1, i_max2) == i &
              & .and. buf_mij_out(2, i_max2) == j &
              & .and. i_max2 /= i_max) then
              mij_flag = .false.
            endif
          enddo
        endif
    ...
    do j = 0, ny
      do i = 0, nx
        if (     vor_part_tmp(i, j) == i_max &
          & .and.vor_part_max(i, j) == 0) then
          vor_part_max(i, j) = i_max
        endif
      enddo
    enddo
